...
const SVBase = require('./SVBase');
const SVFile = require('../DataStruct/SVFile');
const SVNews = require('../DataStruct/SVNews');
const SVNewsDetail = require('../DataStruct/SVNewsDetail');
const SVProfile = require('../DataStruct/SVProfile');
const SVSemester = require('../DataStruct/SVSemester');
const SVSubject = require('../DataStruct/SVSubject');
const SVTimeTableEntry = require('../DataStruct/SVTimeTableEntry');
const SVTimeTable = require('../DataStruct/SVTimeTable');
const SVMarkTable = require('../DataStruct/SVMarkTable');
            ...
            base.Get(Endpoints.Home()).then(function (resp) {
                    ...
                    try {
                        ...
                        $("#ctl05_MyList").find(".important_news A").each(function (k, A) {
                            ...
                            arr.push(new SVNews(id, _class, link, time, title));
                        });
        ...
        return new Promise(function(resolve, reject) {
            ...
            base.Get(link).then(function (resp) {
                ...
                $("#pnlFile ul li a").each(function (k, A) {
                    A = $(A);
                    files.push(new SVFile(A.attr("href"), A.text().trim()));
                });

                resolve(new SVNewsDetail(id, link, time, title, tomtat, content, files));
            }, reject);
        });
    }
    ...
    this.GetProfile = function () {
        return new Promise(function(resolve, reject) {
            base.Get(Endpoints.Make("MarkAndView.aspx")).then(function (resp) {
                ...
                resolve(new SVProfile(id, code, name, _class, major, academicYear, hedaotao));
            }, reject);
        });
    }
                            ...
                            for (var sheetName in sheets) {
                                ...
                                for (var i = 10; i < sheet.length - 9; i++) {
                                    ...
                                    if (row.length == 12 || row.length == 13) {
                                        ...
                                        if (!subject) {
                                            subject = new SVSubject(maMon, tenMon, hocPhan, 0);
                                        ...
                                        for (var pivot = startTime; pivot.getTime() < endTime.getTime(); pivot.setDate(pivot.getDate() + 7)) {
                                            ...
                                            var entry = new SVTimeTableEntry(maMon, pivot, tiets, diaDiem, hinhThuc, giaoVien, dot);
                    ...
                    var processExamData = function (html) {
                        ...
                        $("#tblCourseList tr").each(function (i, tr) {
                            if (i > 0) {
                                ...
                                var subject = new SVSubject(
                                    arr[1],
                                    arr[2],
                                    arr[2],
                                    arr[3]
                                );
                                var entry = new SVTimeTableEntry(
                                    arr[1],
                                    arr[4],
                                    [arr[5]],
                                    arr[8],
